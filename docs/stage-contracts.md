...
`tools/lib/pipeline_run.py` 기준으로 기록합니다.
...
`tools/run-epic001-pipeline.py` / `tools/lib/pipeline_run.py` 는 아래 입력을 받습니다.
...
`tools/lib/pipeline_run.py` 안에서 **Step 02a 결과를 보고 Step 03에 넘길 taint config 경로를 고르는 분기**입니다.
...
### 실행 위치
- `tools/lib/pipeline_run.py` 내부 함수 호출
