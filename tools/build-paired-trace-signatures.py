...
def find_latest_pipeline_run_dir(pipeline_root: Path) -> Path:
    ...
    candidates = [
        p for p in pipeline_root.iterdir()
    ...
    with trace_jsonl.open("r", encoding="utf-8") as f:
        for lineno, line in enumerate(f, start=1):
            ...
            if not testcase_key or not trace_file_raw or not best_flow_type:
                raise ValueError(
                    f"Missing required keys at line {lineno} in {trace_jsonl}: {obj}"
                )
            records.append(
                StrictTraceRecord(
...
def make_pair_id(testcase_key: str, b2b_record: StrictTraceRecord,
                 counterpart_record: StrictTraceRecord) -> str:
    seed = "||".join(
        [
            testcase_key,
...
def main() -> int:
    ...
    for testcase_key, records in sorted(grouped.items()):
        ...
        counterpart_records = [
            r for r in records if r.best_flow_type in COUNTERPART_FLOW_TYPES
        ]
