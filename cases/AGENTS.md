...
이 문서는 `cases/<project>__<CVE>/{vulnerable,patched}/README.md` 작성 규칙을 다룹니다.
...
- 이 번호는 아래 `## trace 구성 방식`과 `## infer로 도출되지 않아 수동 분석한 구간`에서 그대로 사용합니다.
...
- `## 전체 trace`를 작성할 때는 **반드시 `cases/Real_Vul_data.csv`의 해당 케이스 `processed_func` 값을 기준 원문으로 삼습니다.**
- 즉 step의 개수, 순서, 각 줄의 내용은 `processed_func`를 **엄격히 그대로 따르며**, 임의로 재구성, 축약, 정규화, 병합, 분할하지 않습니다.
- `infer` 결과나 수동 분석 메모를 참고하더라도, `## 전체 trace` 자체는 먼저 `processed_func`와 일치해야 합니다. 불일치가 보이면 README 쪽 표현을 맞추고, 필요한 해설은 아래 설명 섹션에서 덧붙입니다.
- `## 전체 trace`에서는 `@file:line` 형태의 anchor를 붙이지 않습니다.
  - line anchor는 아래 `## infer로 도출되지 않아 수동 분석한 구간` 같은 설명 섹션에서 적습니다.
...
- 근거: 전체 trace는 `infer`가 바로 주는 한 개의 trace보다, 여러 구간 trace와 수동 분석을 합쳐 구성되는 경우가 많습니다.
- 표는 아래 형식을 사용합니다.
...
- `infer로 추출` 행의 근거에는 slice 파일의 상대 경로만 적습니다.
  - 예: `../runs/run-002/outputs/06_trace_slices/slice/slice_1f05e87c4707874c.c`
- `infer로 추출` 행의 비고에는 해당 추출의 signature JSON 상대 경로를 적습니다.
...
- `수동 분석` 행의 근거에는 `infer` 직접 제공이 끊긴 원인만 적습니다.
...
| 전체 trace 기준 위치 | 구간 trace 도출 방식 | 근거 | 비고 |
| --- | --- | --- | --- |
| 1 -> 2 | infer로 추출 | `../runs/run-001/outputs/06_trace_slices/slice/slice_a1b2c3d4.c` | `../runs/run-001/outputs/03_signatures/infer-2026.04.14-14:28:35/signature-2026.04.14-14:28:42/non_empty/spice-vdagent/2.json` |
| 2 -> 4 | 수동 분석 | 함수 포인터 필드의 concrete callee 복원 한계 | - |
| 4 -> 5 | infer로 추출 | `../runs/run-002/outputs/06_trace_slices/slice/slice_1f05e87c4707874c.c` | `../runs/run-002/outputs/03_signatures/infer-2026.04.14-14:27:53/signature-2026.04.14-14:28:00/non_empty/spice-vdagent/1.json` |
| 5 -> 6 | infer로 추출 | `../runs/run-003/outputs/06_trace_slices/slice/slice_9e8d7c6b.c` | `../runs/run-003/outputs/03_signatures/infer-2026.04.14-14:27:01/signature-2026.04.14-14:27:08/non_empty/spice-vdagent/1.json` |
```

## 원칙 4. `## infer로 도출되지 않아 수동 분석한 구간`은 실제 연결 구조와 `infer` 한계를 함께 설명합니다.

- 근거: `수동 분석` 구간은 왜 그 연결을 최종 trace에 넣었는지까지 보여줘야 설득력이 생깁니다.
- `수동 분석` 표 행은 같은 위치 기준으로 바로 아래 `## infer로 도출되지 않아 수동 분석한 구간`에서 이어서 설명합니다.
- 각 항목은 헤딩 3으로 표와 같은 위치를 먼저 적고 아래 순서로 설명합니다.
  1. 끊기는 구간
  2. `infer` 한계
  3. 코드상 실제 연결 방식
- `infer` 한계는 자동 추출이 멈춘 지점을 먼저 보여 주는 역할을 합니다.
